...
void build_up_b(float *b, float rho, float dt, float *u, float *v, float dx,
                float dy) {
  /*
      ...
      b[(idx + 1) * nx + idy + 1] =
          (rho * (1 / dt *
                      ((u[(idx + 1) * nx + idy + 2] - u[(idx + 1) * nx + idy]) /
                           (2 * dx) +
                       (v[(idx + 2) * nx + idy + 1] - v[(idx)*nx + idy + 1]) /
                           (2 * dy)) -
                  pow(((u[(idx + 1) * nx + idy + 2] - u[(idx + 1) * nx + idy]) /
                       (2 * dx)),
                      2) -
                  2 * ((u[(idx + 2) * nx + idy + 1] - u[(idx)*nx + idy + 1]) /
                       (2 * dy) *
                       (v[(idx + 1) * nx + idy + 2] - v[(idx + 1) * nx + idy]) /
                       (2 * dx)) -
                  pow((v[(idx + 2) * nx + idy + 1] - v[(idx)*nx + idy + 1]) /
                          (2 * dy),
                      2)));
    }
}
...
void cavity_flow(int nt, float *u, float *v, float dt, float dx, float dy,
                 float *p, float rho, float nu) {
  /*
...
int main() {
  ...
  printf("CPU Elapsed time: %3.3f s\n", elapsed.count());
